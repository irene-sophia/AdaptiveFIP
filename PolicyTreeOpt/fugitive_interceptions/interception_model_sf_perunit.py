...
class FugitiveInterception():
    def __init__(self, T, U, R, graph, units_start, nodesdict_perunit_sorted, fugitive_start, num_sensors,
                 sensor_locations, fugitive_routes_db, multiobj=False, seed=1):
        self.T = T
        self.U = U
        self.R = R
        self.graph = graph
        ...
        for r in range(R):
            ...
            for t in range(1, T):
                ...
                for u in range(U):
                    ...
                    # action, rules = P.evaluate(states=[int(any(sensor_detections['sensor' + str(s)][:t])) for s in range(
                    #    self.num_sensors)])  # states = list of current values of" ['Minute', 'SensorA']  # 16/5 no time experiment
                    ...
                    try:
                        if units_plan[unit_affected][-1] != target_node:
                            if nx.has_path(G=self.graph, source=units_current[unit_affected],
                                ...
                                units_plan[unit_affected] = nx.shortest_path(G=self.graph,
                                                                             ...
                                                                             target=self.nodesdict_perunit_sorted[
                                                                                 unit_affected][target_node])
                        ...
                        if nx.has_path(G=self.graph, source=units_current[unit_affected],
                            ...
                            units_plan[unit_affected] = nx.shortest_path(G=self.graph,
                                                                         source=units_current[unit_affected],
                                                                         target=
                                                                         self.nodesdict_perunit_sorted[unit_affected][
                                                                             target_node])
            ...
            for r in range(R):
                ...
                for u in range(U):
                    interception_dict_perroute[f'unit{u}'] = [i for i, j in
                                                              zip(unit_routes_final[f'route{r}'][f'unit{u}'],
                                                                  fugitive_routes[r])
                                                              if ((i == j) and (
