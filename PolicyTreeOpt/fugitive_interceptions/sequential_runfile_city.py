...
from ptreeoptperunit.fugitive_interceptions.interception_model_city import FugitiveInterception
...
def run_instance(city, U, R, num_sensors, num_seeds):
    ...
    fugitive_start = pd.read_pickle(
        f"../../data/city/sp_const_fugitive_start_{city}_T{t_max}_R{R}_U{U}_numsensors{num_sensors}_instance{instance}.pkl")
    units_start = pd.read_pickle(
        f"../../data/city/sp_const_units_start_{city}_T{t_max}_R{R}_U{U}_numsensors{num_sensors}_instance{instance}.pkl")
    sensor_locations = pd.read_pickle(
        f"../../data/city/sp_const_sensors_{city}_T{t_max}_R{R}_U{U}_numsensors{num_sensors}_instance{instance}.pkl")
    fugitive_routes = pd.read_pickle(
        f"../../data/city/sp_const_fugitive_routes_{city}_T{t_max}_R{R}_U{U}_numsensors{num_sensors}_instance{instance}.pkl")
    ...
    for seed_rep in range(num_seeds):
        ...
        model = FugitiveInterception(T, U, R, graph=graph,
                                     ...
                                     fugitive_start=fugitive_start, fugitive_routes_db=fugitive_routes_db, fugitive_routes=fugitive_routes,
                                     num_sensors=num_sensors, sensor_locations=sensor_locations,
                                     seed=seed_rep)

        algorithm = PTreeOpt(model.f, num_units=U,
                             feature_bounds=[[0, int(T/30)]] + [[0.5, 1.5]] * num_sensors,  # indicators
    ...
    results_instance.to_csv(f'results/results_{city}__R{R}_U{U}_numsensors{num_sensors}_instance{instance}.csv',
                            index=False)
