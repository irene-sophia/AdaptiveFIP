...
def run_rep(args):
    ...
    for time_step in range((int(t_max / 30) + 1)):
        if time_step == 0:  # time_step == 0
            ...
            for u, pol in enumerate(police_current):
                if pol == actions[u]:  # if police unit u has arrived at its target node
                    for fug_t, fug_node in {fug_t_: node for fug_t_, node in ground_truth_route.items() if time_interval[0] <= fug_t_ <= time_interval[1]}.items():
                        if fug_node == actions[u]:  # if the ground truth route passes the target node in the time interval
                            try:
                                if fug_t >= min([pol_t for pol_t, pol_node in police_routes[rep][u].items()
                                             if (pol_node == actions[u])]):  # if police unit u has arrived at its target node before the fug
                                    intercepted = True
        ...
        print('pol current: ', police_current)
        ...
        prev_num_routes_available = len(fugitive_routes_labeled)
        ...
        if (time_step == 0) or (prev_num_routes_available > len(fugitive_routes_labeled)):
