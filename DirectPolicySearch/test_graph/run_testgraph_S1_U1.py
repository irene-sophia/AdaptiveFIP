...
from fugitive_interception_model_S1_U1 import fugitive_interception_model
from DPS.rbf import rbf_cubic

ema_logging.log_to_stderr(ema_logging.INFO)

if __name__ == '__main__':
    for num_units in range(1, 10):
        for seed in range(10):
            try:
                shutil.rmtree('./test_graph/results/tmp')
            except OSError as e:
                pass
            ...
            sensor_location = labels[sensor_location]
            ...
            levers, constants, outcomes = get_problem_formulation(mode, graph, police_start, n_realizations, t_max,
                                                                  sensor_location, fugitive_routes_labeled,
                                                                  rbf_cubic,
            ...
            convergence_metrics = [
                ArchiveLogger(
                    f"./test_graph/results/",
                    [l.name for l in model.levers],
                    [o.name for o in model.outcomes if o.kind != o.INFO],
                    base_filename=f"archives_S1_U1_{graph_type}_seed{seed}.tar.gz"
                ),
            ...
            with SequentialEvaluator(model) as evaluator:
                results = evaluator.optimize(algorithm=GenerationalBorg,
                                             nfe=5e2, searchover='levers',
                                             epsilons=[0.1, ] * len(model.outcomes),
                                             convergence=convergence_metrics, convergence_freq=10)

            convergence = ArchiveLogger.load_archives(f"./test_graph/results/archives_S1_U1_{graph_type}_seed{seed}.tar.gz")
            ...
            convergence_df.to_csv(f'./test_graph/results/archives_S{1}_U{1}_{graph_type}_seed{seed}.csv')
