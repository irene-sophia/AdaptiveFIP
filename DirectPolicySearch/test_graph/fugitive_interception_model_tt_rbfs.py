...
def fugitive_interception_model(
    ...
    for _ in range(num_rbfs):
        for _ in range(len(sensor_location)):
            ...
            # c_i.append(f'c{counter}')
            # r_i.append(f'r{counter}')
            counter += 1
    ...
    for timestep in range(t_max):
        for u, _ in enumerate(police_start):
            ...
            # at = at * len(labels_perunit_sorted[u])
    ...
    if mode == 'simulation':
        actions_df.to_csv(f'./results/RESULTS4OCT_actions_tt_{rbf.__name__}S{len(sensor_location)}_U{len(police_start)}.csv')

        pickle.dump(police_path, open(
            f'./results/RESULTS4OCT_police_path_tt_{rbf.__name__}_S{len(sensor_location)}_U{len(police_start)}.pkl',
            'wb'))

        pickle.dump(interceptions, open(
            f'./results/RESULTS4OCT_interceptions_tt_{rbf.__name__}_S{len(sensor_location)}_U{len(police_start)}.pkl',
            'wb'))
