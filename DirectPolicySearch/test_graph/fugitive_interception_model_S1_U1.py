...
def fugitive_interception_model(
    ...
    sensor_detections = np.zeros((n_realizations, t_max))
    ...
    for t in range(t_max):
        ...
        if t == 0:
            ...
            for t_sensor in range(t + 1):
                a = a + np.where(fugitive_routes[:, t_sensor] == sensor_location, 1, 0)
            sensor_detections[:, t] = a
        # evaluate rbf to obtain at (=target_node)
        at = rbf(xt=sensor_detections[:, t], c_lm=c1, r_lm=r1, w_md=w1, labels_perunit_sorted=labels_perunit_sorted)
    ...
    if mode == 'simulation':
        ...
        actions_df.to_csv(f'./results/actions_S{1}_U{1}.csv')
        police_path_output.to_csv(f'./results/police_path_S{1}_U{1}.csv')
